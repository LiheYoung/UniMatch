...
class ACDCDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train_l':
            return torch.from_numpy(img.transpose(2,0,1)).float(), torch.from_numpy(np.array(mask)).long()
        ...
        return torch.from_numpy(img).float(), torch.from_numpy(img_s1).float(), torch.from_numpy(img_s2).float(), cutmix_box1, cutmix_box2
